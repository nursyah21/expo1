...
function FollowingScreen({setVisitUser}){
  const [loading, setLoading] = useState(false)
  ...
  useEffect(()=>{
    (async function(){
      try{
        ...
        let { data: followers, error } = await supabase.from('followers').select('following_id').eq('follower_id', user.id)
        if(error) throw error

        let {data: users} = await supabase.from('users').select('id,name')

        let myFollowers = users.filter((u) => followers.find(f => f.following_id === u.id ))
        // numbers.filter((number) => number % 2 === 0)
        
        // console.log(myFollowers)
        setResult(myFollowers)
        setMessage('')
      }catch{
        setResult([])
      }
          ...
          <DataTable>
            ...
            {result.map((e, idx)=>
              <TouchableOpacity key={idx} onPress={()=>{
                ...
                navigation.navigate('VisitedTab2')
              }}>
                <DataTable.Row  >
...
function Screen({session}){
    ...
    return (
      <Tab.Navigator>
          <Tab.Screen name='FollowingTab' options={{tabBarLabel: 'Following'}} >
            {(props) => <FollowingScreen {...props} key={Math.random()} setVisitUser={setVisitUser} /> }
          </Tab.Screen>
          <Tab.Screen name='VisitedTab2'  options={{tabBarLabel: 'Visited'}} >
